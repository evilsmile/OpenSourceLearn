...
reverse_complement(Buffer) ->
    {Chunks, Left} = calculate_split(size(Buffer), ?WORKERS),
...
start_jobs(Buffer, Chunk, Size, N) when Size >= Chunk ->
    new_job({fun revcmp_chunk/4, [Buffer, Size - Chunk, Size, << >>]}, N),
    start_jobs(Buffer, Chunk, Size - Chunk, N + 1).

revcmp_chunk(_, Start, Start, Acc) ->
    Acc;
revcmp_chunk(Buffer, Start, Stop, Acc) ->
    From = Stop - ?WIDTH,
    << _:From/bytes, Line:?WIDTH/bytes, _/bytes >> = Buffer,
    RC = revcmp(Line),
    revcmp_chunk(Buffer, Start, From, << Acc/binary, RC/binary>>).
