...
static char* ngx_http_myfilter_set(ngx_conf_t *cf, ngx_command_t *cmd, void *ctx);
...
static ngx_int_t ngx_http_myfilter_body_filter(ngx_http_request_t *r, ngx_chain_t *c);
...
static ngx_int_t ngx_http_myfilter_post_config(ngx_conf_t *cf)
{
    ...
    ngx_conf_log_error(NGX_LOG_INFO, cf, 0, "myfilter post config done.");
...
static char* ngx_http_myfilter_set(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    ngx_conf_log_error(NGX_LOG_INFO, cf, 0, "arg_flag = %d", mlcf->arg_flag);
...
static void* ngx_http_myfilter_create_loc_conf(ngx_conf_t* cf)
{
    ...
    mlcf = ngx_palloc(cf->pool, sizeof(ngx_http_myfilter_loc_conf_t));
    if (mlcf == NULL) {
        ngx_conf_log_error(NGX_LOG_ERR, cf, 0, "create http_myfilter_loc_conf_t failed.");
        return NULL;
    }
...
static ngx_int_t ngx_http_myfilter_header_filter(ngx_http_request_t* r)
{
    if (r->headers_out.status != NGX_HTTP_OK) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "headers_out.status = %d", r->headers_out.status);
    ...
    // flag 为0则表示不加前缀
    if (mlcf->arg_flag == 0) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "arg_flag = 0");
        return ngx_http_next_header_filter(r);
    }

    ngx_http_myfilter_ctx_t *mctx;
    mctx = ngx_http_get_module_ctx(r, ngx_http_myfilter_module);
    //mctx不为空，表示这个头部已经处理过，不许要再处理了  
    if (mctx != NULL) {
        return ngx_http_next_header_filter(r);
    }

    mctx = ngx_palloc(r->pool, sizeof(ngx_http_myfilter_ctx_t));
    if (mctx == NULL) {
        return NGX_ERROR;
    }
    ...
    //初始化为不加前缀
    mctx->flag_count = 0; 
    // 原本是只针对 头部类型为“text/plain”的要加前缀。 现在放开
...
static ngx_int_t ngx_http_myfilter_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    if (mctx == NULL || mctx->flag_count != 1) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "mctx null or flag_count != 1: [%p]", mctx);
        return ngx_http_next_body_filter(r, in);
    }

    mctx->flag_count = 2;
    ngx_buf_t* buf = ngx_create_temp_buf(r->pool, filter_prefix.len);
    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "filter prefix: %s", filter_prefix.data);
