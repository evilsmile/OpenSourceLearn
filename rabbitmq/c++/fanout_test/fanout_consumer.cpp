...
void single_queue_init(amqp_connection_state_t& conn, const std::string& exchange_name, const std::string& queue_name, const std::string& route_name)
{
    ...
    snprintf(szChannelId, sizeof(szChannelId), "%d", LOCAL_CHANNEL_ID);
...
void rabbit_init(amqp_connection_state_t& conn)
{
    ...
    amqp_channel_open(conn, LOCAL_CHANNEL_ID);
    check_amqp_reply(conn, "amqp open channel failed.");
    
    amqp_exchange_declare(conn, /* state */
                          LOCAL_CHANNEL_ID,    /* channel */
                          amqp_cstring_bytes(LOCAL_EXCHANGE.c_str()),  /* exchange */
                          amqp_cstring_bytes(LOCAL_EXCHANGE_TYPE.c_str()),  /* type */
                          0,          /* passive */
                          1,          /* durable */
                          0,          /* auto_delete */
                          0,    /* internal */
    ...
    std::cout << "Exchange '" << ALERT_EXCHANGE << "' declared." << std::endl;

    single_queue_init(conn, LOCAL_EXCHANGE, LOCAL_QUEUE_1, "");
    single_queue_init(conn, LOCAL_EXCHANGE, LOCAL_QUEUE_2, "");
}
...
{
    amqp_channel_close(conn, LOCAL_CHANNEL_ID, AMQP_REPLY_SUCCESS);
